...
public class GetMessagesByChatIdQueryHandler(IMessagesRepository messagesRepository) : IRequestHandler<GetMessagesByChatIdQuery, IReadOnlyList<Message>>
{
    public async Task<IReadOnlyList<Message>> Handle(GetMessagesByChatIdQuery request, CancellationToken cancellationToken)
    {
        return await messagesRepository.GetByChatIdAsync(request.ChatId);
    }
}